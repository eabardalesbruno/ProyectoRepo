...
import com.proriberaapp.ribera.Infraestructure.repository.activity.ActivityDshboardRepository;
...
public class ActivityDashboardServiceImpl implements ActivityDashboardService {
    private final ActivityDshboardRepository activityDashboardRepository;
    ...
    private Mono<RoomDetailResult> getRoomDetails(LocalDateTime date, int page, int size) {
        return activityDashboardRepository.findAllRooms(date, size, page)
                ...
                .collectList()
                .map(rooms -> new RoomDetailResult(
                        rooms,
                        PaginationDTO.builder()
                                .currentPage(page)
                                .size(size)
                                .totalElements((long) rooms.size())
                                .totalPages((int) Math.ceil(rooms.size() / (double) size))
                                .build()));
    }
    ...
    private RoomDetailDTO mapToRoomDetailDTO(ActivityRoomProjection projection) {
        return RoomDetailDTO.builder()
                ...
                .reservation(projection.getBookingId() != null ? mapToReservationDetail(projection) : null)
        ...
        return ReservationDetailDTO.builder()
                ...
                .capacity(RoomCapacityDTO.builder()
                        ...
                        .total(projection.getNumberAdults() + projection.getNumberChildren()
                                + projection.getNumberBabies() + projection.getNumberAdultsExtra() +
